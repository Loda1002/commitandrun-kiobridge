...
export function explainAlternative(result: EngineResult, candidateId: string): string {
  ...
  if (worse.length === 0 && better.length === 0) {
    ...
    //
    // 2026-08-16: 「1등과 맞춘 조건이 같습니다. 맞춘 조건만으로는 순위가 갈리지
    // 않습니다.」였다. 「1등」은 우리가 코드에서 쓰는 말이고, 두 문장이 같은 말을
    // 두 번 한다(팀장 지시: 화면 문장이 사람 말 같지 않다). 뜻은 그대로 두되,
    // 읽는 사람이 그래서 무엇을 하면 되는지까지 한 문장으로 말한다.
    return "추천드린 것과 맞는 조건이 같습니다. 어느 쪽을 고르셔도 됩니다.";
  }
